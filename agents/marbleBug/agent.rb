...
class State
  attr_accessor :messages, :game_over, :killer_bug, :was_stunned, :was_killed, :killed_someone, :round_changed, :player, :rows, :columns, :board
...
class Agent
  ...
  def run(hostname, port)
    ...
    loop do
	  ...
	  @state.round_changed = ((flag & 0x10) == 0x10)
      (0...rows).each { |r|
        (0...columns).each { |c|
	  ...
	  flag_text << "[Round changed]" if @state.round_changed
